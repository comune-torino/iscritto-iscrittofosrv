package it.csi.iscritto.iscrittofosrv.business.nido.impl;
...
import it.csi.iscritto.iscrittofosrv.business.nido.DomandaNidoApi;
...
public class DomandaNidoApiServiceImpl extends SpringSupportedResource implements DomandaNidoApi {
	...
	@Override
	public Response saveBozza(SecurityContext sc, HttpServletRequest request, String cfRichiedente, DomandaNido domandaNido) {
		final String methodName = "saveBozza";
		log.debug(buildMessage(getClass(), methodName, LOG_BEGIN));

		Response response;
		try {
			Errors errors = this.domandaValidationService.validateSoggetto3(domandaNido);
			if (errors.hasErrors()) {
			...
			CallerInfo callerInfo = CallerInfoBuilder.from(sc, request)
					...
					.oggOper(methodName)
					.keyOper(buildKeyOper(cfRichiedente, domandaNido))
					.build();

			DomandaNido domandaNidoNew = this.domandaSrvBean.insertDomandaNido(callerInfo, cfRichiedente, domandaNido);
			response = Response.ok(new ResponseBody<>(buildSuccessStatus(), domandaNidoNew)).build();
		}
		catch (Exception e) {
			log.error(buildMessage(getClass(), methodName, LOG_ERROR), e);
			return buildErrorResponse(e);
	...
	public Response invioDomanda(SecurityContext sc, HttpServletRequest request, Long idDomandaIscrizione, String cfRichiedente) {
		...
		try {
			...
			DomandaNido domandaNido = this.domandaSrvBean.getDomandaNido(callerInfo1, cfRichiedente, idDomandaIscrizione);
			if (Boolean.TRUE.equals(domandaNido.getNao())) {
				InfoNao infoNao = this.visuraFamigliaServiceNao.dettaglioFamigliaPerCodiceFiscale(
						callerInfo1, domandaNido.getRichiedente().getAnagrafica().getCodiceFiscale());

				Errors errors = this.naoValidationService.validateSoggettiNao(infoNao, domandaNido);
				if (errors.hasErrors()) {
					ServiceException e = new ServiceException(errors);
					log.error(buildMessage(getClass(), methodName, LOG_ERROR), e);
			...
			String protocollo = this.domandaSrvBean.invioDomandaNido(callerInfo2, cfRichiedente, idDomandaIscrizione);

			DatiRicevuta datiRicevuta = new DatiRicevuta();
			datiRicevuta.setProtocollo(protocollo);
			datiRicevuta.setAnnoScolastico(domandaNido.getAnnoScolastico());
	...
	public Response read(SecurityContext sc, HttpServletRequest request, Long idDomandaIscrizione, String cfRichiedente) {
		...
		try {
			...
			DomandaNido result = this.domandaSrvBean.getDomandaNido(callerInfo, cfRichiedente, idDomandaIscrizione);
	...
	@Override
	public Response getElencoDomandeNidiByCfRichiedente(SecurityContext sc, HttpServletRequest request, String codiceFiscale) {
		final String methodName = "getElencoDomandeNidiByCfRichiedente";
		log.debug(buildMessage(getClass(), methodName, LOG_BEGIN));
		...
		try {
			...
			List<InfoDomanda> result = this.domandaSrvBean.getElencoDomandeByCfRichiedente(callerInfo, TipoScuola.NID, codiceFiscale);
			response = Response.ok(new ResponseBody<>(buildSuccessStatus(), result)).build();
		}
		catch (Exception e) {
			log.error(buildMessage(getClass(), methodName, LOG_ERROR), e);
	...
	public Response verificaDomanda(SecurityContext sc, HttpServletRequest request, String cfRichiedente, String cfMinore) {
		...
		try {
			...
			this.domandaSrvBean.checkDomandaNido(callerInfo, cfRichiedente, cfMinore);
	...
	public Response getTestataDomandaAccettazione(SecurityContext sc, HttpServletRequest request, Long idDomandaIscrizione, String cfRichiedente) {
		...
		try {
			...
			TestataDomandaAccettazione result = this.domandaSrvBean.getTestataDomandaAccettazione(
					callerInfo, idDomandaIscrizione, cfRichiedente, TipoScuola.NID);
	...
	public Response accettaPreferenza(SecurityContext sc, HttpServletRequest request, Long idDomandaIscrizione, String codiceScuola, String codiceTipoFrequenza,
		...
		try {
			...
			TestataDomandaAccettazione result = this.domandaSrvBean.accettaPreferenza(
					callerInfo, TipoScuola.NID, idDomandaIscrizione, codiceScuola, codiceTipoFrequenza, datiAccettazionePreferenza);
	...
	private static String buildKeyOper(String cfRichiedente, DomandaNido domandaNido) {
		...
		if (domandaNido != null) {
			String cfOperatore = domandaNido.getCodiceFiscaleOperatore();
